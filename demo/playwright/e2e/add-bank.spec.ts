...
test.describe(
  "As a Taxpayer user, I should be able to save and delete bank account information",
  () => {
    for (let i = 0; i < 10; i++) {
      test(`Initiating test for account ${i}`, async ({ page }) => {
        ...
        await addBankAccountDetails(page);
        await savePaymentMethodAndFinishPayment(page);
        await closeApplicationConfirmation(page);

        await openProfile(page);
        const savedBankAccountsLength = await countSavedBankAccounts(page);
        void savedBankAccountsLength;
      });
    }
  }
);