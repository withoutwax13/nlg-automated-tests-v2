...
describe.skip("After the reviewer approved an application, taxpayer user must receive an email with subject, content, and HTML structure similar to the editable template in the workflow.'", () => {
  it("Initiating test", () => {
    ...
    cy.get("@referenceId").then((referenceId) => {
      cy.request("GET", `${ENDPOINT}&livequery=true`).then((response) => {
        ...
        cy.wrap(email.subject).should(
          "equal",
          "An Application Has Been Approved on Localgov"
        );
        cy.wrap(email.html).should("include", "Application Status:</span> Approved");
        cy.wrap(email.html).should(
          "include",
          `Reference ID:</span>\t${String(referenceId).toUpperCase()}`
        );
        cy.wrap(email.html).should("include", `Form Name:</span>\t${expectedFormName}`);
        // cy.wrap(email.html).should(
        //   "include",
