...
describe("As a Government User, if the business user's application meet a specific condition set in the Certificate builder, the Registration Record should have a certificate button under the Active Registration Card when it's registration status is changed to ACTIVE or approved via Application Reviewer.", () => {
  it("Initiating test", () => {
    ...
    cy.getUniqueRegistrationData(randomSeed(), false, undefined, {
      ...
      (customData: {
        ...
        cy.get("@registrationRecordId").then((registrationRecordId) => {
          ...
          cy.login({ accountType: "taxpayer", notFirstLogin: true, accountIndex: 6 });
          taxpayerApplicationGrid.init();
          ...
          registrationGrid.toggleRegistrationActionButton(
            "View",
            "Registration Record ID",
            String(registrationRecordId)
          );
