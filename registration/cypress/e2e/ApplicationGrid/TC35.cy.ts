...
describe("As a Government User, if the business user's application does not meet a specific condition set in the Certificate builder, the Application Record should not have a certificate button when it's registration status is Approved", () => {
  it("Initiating test", () => {
    ...
    cy.getUniqueRegistrationData(randomSeed(), false).then(
      (customData: {
        ...
        cy.logout();
        cy.login({ accountType: "ags", accountIndex: 1, notFirstLogin: true });
        ...
        cy.get("@registrationRecordId").then((registrationRecordId) => {
          ...
          agsApplicationGrid.getElementOfColumn(
            "Certificate",
            "Registration Record ID",
            String(registrationRecordId),
            "certificateButton"
          );
          cy.get("@certificateButton").find("svg").should("not.exist");
        });
      }
    );
  });
