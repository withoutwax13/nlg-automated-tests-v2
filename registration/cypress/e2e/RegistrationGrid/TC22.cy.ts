...
describe("As a Government User, when a Registration Record is created and added to my Registration List, I can see a unique “Registration Record ID” in the data grid column.", () => {
  it("Initiating test", () => {
    ...
    cy.getUniqueRegistrationData(randomSeed(), false).then(
      (customData: {
        ...
        cy.get("@registrationRecordId").then((registrationRecordId) => {
          ...
          registrationGrid.filterColumn("Registration Record ID", String(registrationRecordId));
          registrationGrid.getElement().rows().should("have.length", 1);
        });
      }
    );
  });
