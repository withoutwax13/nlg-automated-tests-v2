...
describe("As a Government User, if the business user's application does not meet a specific condition set in the Certificate builder, the Registration Record should not have a certificate button when it's registration status is changed to ACTIVE.", () => {
  it("Initiating test", () => {
    ...
    cy.getUniqueRegistrationData(randomSeed(), false).then(
      (customData: {
        ...
        cy.get("@registrationRecordId").then((registrationRecordId) => {
          ...
          registrationGrid.getElementOfColumn(
            "Certificate",
            "Registration Record ID",
            String(registrationRecordId),
            "certificateButton"
