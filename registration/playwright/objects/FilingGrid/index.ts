...
class FilingGrid {
  ...
  deleteFiling(anchorColumnName: string, anchorValue: string) {
    if (this.userType === "taxpayer") {
      ...
      pw.get(".k-dialog-actions").find("button").contains("Delete").click( {force: true} );
          ...
          this.getElement()
            ...
            .each(($row) => {
              ...
              if (
                ...
                pw.wrap($columns.eq(columnIndex)).find("button").click( {force: true} );
                this.getElement().anyList().contains("Delete Filing").click( {force: true} );
                // TODO: Add delete confirmation POM
  ...
  updateStatus(
    ...
    pw.get(".k-window-content")
      ...
      .click( {force: true} );
    pw.get(".k-dialog-actions").find("button").contains("Save").click( {force: true} );
    pw.waitForLoading();
  }
  ...
  checkAuditLog(anchorColumnName: string, anchorValue: string) {
    ...
    pw.get(
      ...
      .find("button")
      .click( {force: true} );
    this.getElement().anyList().contains("Audit Log").click( {force: true} );
    pw.waitForLoading();
  }
  ...
  clickViewRequestedExtractButton() {
    this.getElement().viewRequestedExtractButton().click( {force: true} );
    pw.waitForLoading(5);
  }
  ...
  setStartDate({
    ...
    pw.get(".fa-calendar-days").click( {force: true} );
    pw.get(".k-animation-container input")
      .first()
      .type(`${month}{rightArrow}${day}{rightArrow}${year}`);
    pw.get(".k-animation-container").find("button").contains("Filter").click( {force: true} );
    pw.waitForLoading();
  }
  ...
  addCustomField(customFieldTitle: string, customFieldName: string) {
    if (this.userType === "ags") {
      ...
      pw.get("button").contains("Add New Column").click( {force: true} );
      ...
      pw.get("button").contains("Save").click( {force: true} );
