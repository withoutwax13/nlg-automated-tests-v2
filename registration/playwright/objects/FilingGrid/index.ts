...
class FilingGrid {
  ...
  deleteFiling(anchorColumnName: string, anchorValue: string) {
    if (this.userType === "taxpayer") {
      ...
      cy.get(".k-dialog-actions").find("button").contains("Delete").click( {force: true} );
          ...
          this.getElement()
            ...
            .each(($row) => {
              ...
              if (
                ...
                cy.wrap($columns.eq(columnIndex)).find("button").click( {force: true} );
  ...
  updateStatus(
    ...
    cy.get(".k-window-content")
      ...
      .click( {force: true} );
    cy.get(".k-dialog-actions").find("button").contains("Save").click( {force: true} );
    cy.waitForLoading();
  }
  ...
  setStartDate({
    ...
    cy.get(".fa-calendar-days").click( {force: true} );
    cy.get(".k-animation-container input")
      .first()
      .type(`${month}{rightArrow}${day}{rightArrow}${year}`);
    cy.get(".k-animation-container").find("button").contains("Filter").click( {force: true} );
    cy.waitForLoading();
  }
  ...
  addCustomField(customFieldTitle: string, customFieldName: string) {
    if (this.userType === "ags") {
      ...
      cy.get("button").contains("Add New Column").click( {force: true} );
