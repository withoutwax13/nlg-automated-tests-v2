...
test.describe("As municipal user, exported file's last modification date data should match the grid", () => {
  test("Initiate test", () => {
    const columnName = "Last Modification Date";
    const columnDataAlias = "lastModificationDateList";
      ...
      cy.get(`@${columnDataAlias}`).then((columnDataAlias) => {
        columnDataAlias.forEach((aliasStringItem) => {
          const found = excelData.some(
            (row) =>
              row[columnName] === aliasStringItem ||
              row[columnName] === "" ||
              aliasStringItem === ""
          );
