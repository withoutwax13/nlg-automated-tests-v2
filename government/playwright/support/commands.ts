...
Cypress.Commands.add("login", (params) => {
  const auth = {
    taxpayer: {
      validUsername: Cypress.env("validCredentials").taxpayer.username,
      validPassword: Cypress.env("validCredentials").taxpayer.password
    },
    municipal: {
      validUsername: Cypress.env("validCredentials").municipal.username,
      validPassword: Cypress.env("validCredentials").municipal.password
    },
    ags: {
      validUsername: Cypress.env("validCredentials").ags.username,
      validPassword: Cypress.env("validCredentials").ags.password
    }
  }
  ...
  const loginFormMethod = (accountType) => {
    ...
    cy.get('[data-cy="email-address"]').should("have.value", auth[accountType].validUsername);
    cy.get('[data-cy="password"]').type(auth[accountType].validPassword);
    cy.get('[data-cy="password"]').should("have.value", auth[accountType].validPassword);
