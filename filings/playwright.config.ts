...
const isLocalParallelRun = process.env.FILINGS_LOCAL_PARALLEL === 'true';
const isLocalSingleRun = process.env.FILINGS_LOCAL_SINGLE === 'true';
const captureLocalFailureScreenshots =
  process.env.FILINGS_LOCAL_FAILURE_SCREENSHOTS === 'true';
const expectedSlotId = process.env.FILINGS_EXPECTED_SLOT_ID;
...
function requirePathInside(
  ...
  if (
    ...
    throw new Error(
      `${environmentName} must resolve beneath filings/test-results/local/.`,
    );
  }
}
...
function requirePathInsideLocalReports(
  ...
  if (
    ...
    throw new Error(
      `${environmentName} must resolve beneath filings/test-results/local/reports/.`,
    );
  }
}
...
if (isLocalSingleRun && isLocalParallelRun) {
  throw new Error(
    'A Filings run cannot be both local single-slot and local parallel.',
  );
}
if (isLocalParallelRun) {
  if (!expectedSlotId || !/^slot-0[0-9]$/.test(expectedSlotId)) {
    throw new Error(
      'FILINGS_EXPECTED_SLOT_ID must identify slot-00 through slot-09 for local parallel runs.',
    );
  }
  ...
  requirePathInsideLocalReports(resultFile, 'FILINGS_LOCAL_RESULT_FILE');
  ...
  reporters = [
    ...
    [
      require.resolve(
        './playwright/support/local-filings-result-reporter',
      ),
  ...
  if (!expectedSlotId || !/^slot-0[0-9]$/.test(expectedSlotId)) {
    throw new Error(
      'FILINGS_EXPECTED_SLOT_ID must identify slot-00 through slot-09 for local single-slot runs.',
    );
  }
  localSingleOutputDirectory = requiredAbsoluteOutputDirectory(
    'FILINGS_LOCAL_OUTPUT_DIR',
  );
  const htmlOutputDirectory = requiredAbsoluteOutputDirectory(
    'FILINGS_LOCAL_HTML_OUTPUT_DIR',
  );
  requirePathInside(
    localResultsRoot,
...
export default defineConfig({
  ...
  // A missing locator previously consumed 20 minutes. Normal Filings tests
