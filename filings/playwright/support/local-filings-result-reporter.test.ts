...
import LocalFilingsResultReporter, {
...
function main(): void {
  const temporaryDirectory = mkdtempSync(
    path.join(tmpdir(), 'filings-result-summary-'),
  );
  ...
  try {
    ...
    const rootDirectory = path.join(temporaryDirectory, 'filings');
    ...
    const reporter = new LocalFilingsResultReporter({
      slotId: 'slot-00',
      outputFile,
    ...
    const parsedIpcMessages = ipcMessages.map((message) =>
      parseLocalFilingsResultIpcMessage(message, 'slot-00'),
    );
    assert.deepEqual(
    ...
    const sanitizedIpcMessage = parseLocalFilingsResultIpcMessage(
      {
        ...testEndMessage,
    ...
    assert.throws(
      () => parseLocalFilingsResultIpcMessage(testEndMessage, 'slot-01'),
      /Invalid local Filings result IPC message/,
    );
    ...
    assert.throws(
      ...
      /Invalid local Filings result summary/,
    );
    ...
    const configEnvironment = {
      ...
      FILINGS_LOCAL_SINGLE: undefined,
      FILINGS_LOCAL_FAILURE_SCREENSHOTS: 'true',
      FILINGS_EXPECTED_SLOT_ID: 'slot-00',
      FILINGS_LOCAL_RESULT_FILE: path.join(
      ...
      FILINGS_LOCAL_OUTPUT_DIR: undefined,
      FILINGS_LOCAL_HTML_OUTPUT_DIR: undefined,
      E2E_CAPTURE_ARTIFACTS: undefined,
      TESTOMATIO: SECRET_MARKER,
    };
    const localConfig = loadFilingsConfig(configEnvironment);
    ...
    assert.match(
      localReporters[1][0],
      /local-filings-result-reporter(?:\.ts)?$/,
    );
    assert.deepEqual(localReporters[1][1], {
      slotId: 'slot-00',
      outputFile: configEnvironment.FILINGS_LOCAL_RESULT_FILE,
    });
    ...
    const singleConfig = loadFilingsConfig({
      ...configEnvironment,
      FILINGS_LOCAL_PARALLEL: undefined,
      FILINGS_LOCAL_SINGLE: 'true',
      FILINGS_LOCAL_OUTPUT_DIR: singleOutputDirectory,
      FILINGS_LOCAL_HTML_OUTPUT_DIR: singleHtmlDirectory,
    });
    ...
    assert.throws(
      () =>
        loadFilingsConfig({
          ...
          FILINGS_LOCAL_SINGLE: 'true',
          FILINGS_LOCAL_OUTPUT_DIR: singleOutputDirectory,
          FILINGS_LOCAL_HTML_OUTPUT_DIR: path.join(
            temporaryDirectory,
            'outside-html',
          ),
        }),
      /must resolve beneath filings\/test-results\/local/,
    );

    const regularConfig = loadFilingsConfig({
      ...configEnvironment,
      FILINGS_LOCAL_PARALLEL: undefined,
      FILINGS_LOCAL_SINGLE: undefined,
      FILINGS_LOCAL_FAILURE_SCREENSHOTS: undefined,
    });
    ...
    const optedInArtifactConfig = loadFilingsConfig({
      ...configEnvironment,
      FILINGS_LOCAL_PARALLEL: undefined,
      FILINGS_LOCAL_SINGLE: undefined,
      FILINGS_LOCAL_FAILURE_SCREENSHOTS: undefined,
    ...
    assert.throws(
      () =>
        loadFilingsConfig({
          ...configEnvironment,
          FILINGS_LOCAL_RESULT_FILE: 'relative-result.json',
        }),
      /FILINGS_LOCAL_RESULT_FILE must be an absolute/,
    );
    assert.throws(
      () =>
        loadFilingsConfig({
    ...
    assert.throws(
      ...
      /must resolve beneath filings\/test-results\/local\/reports/,
    );
    assert.throws(
      () =>
        loadFilingsConfig({
  ...
  process.stdout.write('Filings local result reporter tests passed.\n');
}
