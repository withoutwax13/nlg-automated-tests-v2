...
export interface LocalFilingsResultReporterOptions {
  slotId?: string;
  outputFile?: string;
...
interface LocalFilingsResultIpcBase {
  type: 'filings-local-result';
  version: 1;
  slotId: string;
  totals: LocalSlotResultTotals;
}

export interface LocalFilingsResultBeginIpcMessage
  extends LocalFilingsResultIpcBase {
  event: 'begin';
  status: 'running';
}

export interface LocalFilingsResultTestEndIpcMessage
  extends LocalFilingsResultIpcBase {
  event: 'test-end';
  status: 'running';
...
export interface LocalFilingsResultEndIpcMessage
  extends LocalFilingsResultIpcBase {
  event: 'end';
...
export type LocalFilingsResultIpcMessage =
  | LocalFilingsResultBeginIpcMessage
  | LocalFilingsResultTestEndIpcMessage
  | LocalFilingsResultEndIpcMessage;
...
export function parseLocalFilingsResultIpcMessage(
  value: unknown,
  expectedSlotId?: string,
): LocalFilingsResultIpcMessage {
  const invalid = (): never => {
    throw new Error('Invalid local Filings result IPC message.');
  };

  if (
    !isObject(value) ||
    value.type !== 'filings-local-result' ||
    value.version !== 1 ||
...
export function parseLocalSlotResultSummary(
  ...
  const invalid = (): never => {
    throw new Error(
      `Invalid local Filings result summary for ${expectedSlotId}.`,
    );
  };
...
export function aggregateLocalSlotResults(
  ...
  if (
    ...
    throw new Error(
      'Expected local Filings slots must be unique slot-00 through slot-09 IDs.',
    );
  }
  if (
    ...
    throw new Error(
      'Local Filings summary staleness must be a positive number of milliseconds.',
    );
  }
  const now = options.now ?? new Date();
  if (Number.isNaN(now.getTime())) {
    throw new Error(
      'Local Filings summary aggregation requires a valid current time.',
    );
  }
 ...
 */
export default class LocalFilingsResultReporter implements Reporter {
  ...
  constructor(options: LocalFilingsResultReporterOptions = {}) {
    const slotId = options.slotId ?? process.env.FILINGS_EXPECTED_SLOT_ID;
    if (!slotId || !SLOT_ID_PATTERN.test(slotId)) {
      throw new Error(
        'The local Filings result reporter requires slot-00 through slot-09.',
      );
    }
    if (
      ...
      throw new Error(
        'The local Filings result reporter output file must be absolute.',
      );
    }
  ...
  private sendIpc(
    event: LocalFilingsResultIpcMessage['event'],
    ...
    const safeMessage = parseLocalFilingsResultIpcMessage(
      candidate,
      this.slotId,
    );
    try {
