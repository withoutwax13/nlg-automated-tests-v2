...
function loadEnvVars(projectRoot: string): Record<string, Any> {
  ...
  out.fixtureData = readJsonIfExists(path.join(projectRoot, "playwright", "fixtures", "data.json"))
    || readJsonIfExists(path.join(projectRoot, "cypress", "fixtures", "data.json"));
  return out;
}
...
export function createCy(page: Page, request: APIRequestContext, projectRoot: string) {
  ...
  const cy: Any = {
    ...
    fixture: (name: string) => runtime.chain(undefined).then(() => {
      ...
      const p2 = path.join(projectRoot, "cypress", "fixtures", `${name}.json`);
      const fp = fs.existsSync(p1) ? p1 : p2;
      return JSON.parse(fs.readFileSync(fp, "utf-8"));
    }),
