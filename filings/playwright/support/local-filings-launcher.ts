...
const BUSINESSES_RESOURCE_SLOT_ENV = 'BUSINESSES_RESOURCE_SLOT_JSON';
const BUSINESSES_EXPECTED_SLOT_ID_ENV = 'BUSINESSES_EXPECTED_SLOT_ID';
...
const LOCAL_FAILURE_SCREENSHOTS_ENV =
  'FILINGS_LOCAL_FAILURE_SCREENSHOTS';
const LOCAL_SINGLE_RUN_ENV = 'FILINGS_LOCAL_SINGLE';
const LOCAL_SINGLE_HTML_ENV = 'FILINGS_LOCAL_HTML_OUTPUT_DIR';
const LOCAL_SINGLE_OUTPUT_ENV = 'FILINGS_LOCAL_OUTPUT_DIR';

const filingsDirectory = path.resolve(__dirname, '..', '..');
const businessesDirectory = path.resolve(filingsDirectory, '..', 'businesses');
export const LOCAL_FILINGS_RESULTS_ROOT = path.join(
  filingsDirectory,
  'test-results',
  'local',
);
export const LOCAL_FILINGS_RESULTS_LOCK = path.join(
  businessesDirectory,
  '.local-businesses-results.lock',
);
export const LOCAL_FILINGS_ENV_FILE = path.join(
  businessesDirectory,
...
export function resetLocalFilingsResults(
  resultsRoot: string = LOCAL_FILINGS_RESULTS_ROOT,
  expectedFilingsDirectory: string = filingsDirectory,
): void {
  const expectedRoot = path.resolve(
    expectedFilingsDirectory,
    'test-results',
    'local',
  );
  ...
  if (resolvedRoot !== expectedRoot) {
    throw new Error(
      'Refusing to reset anything except the managed Filings local results directory.',
    );
  }

  const resolvedFilingsDirectory = path.resolve(expectedFilingsDirectory);
  assertOrdinaryDirectory(
    resolvedFilingsDirectory,
    'The Filings project directory',
  );
  const realFilingsDirectory = realpathSync(resolvedFilingsDirectory);
  ...
  if (existsSync(testResultsDirectory)) {
    assertOrdinaryDirectory(
      ...
      'filings/test-results',
    );
    if (
      realpathSync(testResultsDirectory) !==
      path.join(realFilingsDirectory, 'test-results')
    ) {
      throw new Error(
        'Refusing to reset filings/test-results through an unexpected filesystem path.',
      );
    }
  } else {
...
export function acquireLocalResultsLease(
  lockDirectory: string = LOCAL_FILINGS_RESULTS_LOCK,
): LocalResultsLease {
  ...
  if (resolvedLockDirectory !== expectedLockDirectory) {
    throw new Error('The shared local E2E results lock path is invalid.');
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        'Another local Businesses or Filings run owns the shared test resources. Wait for it to finish. ' +
          'If no run is active, remove businesses/.local-businesses-results.lock and retry.',
      );
    }
    throw new Error('Unable to acquire the shared local E2E results lock.');
  }
  ...
  try {
    ...
    throw new Error('Unable to initialize the shared local E2E results lock.');
  }
...
export function createLocalSingleRunPaths(
  ...
  if (
    ...
    throw new Error('Unable to create valid local Filings single-run paths.');
  }
  const timestamp = now
    .toISOString()
    .replace(/[-:]/g, '')
...
export function parseLocalLauncherArguments(
  ...
  for (const argument of argumentsToParse) {
    if (argument === '--slot') {
      throw new Error(
        'The local Filings launcher requires --slot=slot-NN (with an equals sign).',
      );
    }

    if (argument.startsWith('--slot=')) {
      if (slotId !== undefined) {
        throw new Error('The local Filings launcher accepts exactly one --slot argument.');
      }
    ...
    if (isManagedPlaywrightArgument(argument)) {
      throw new Error(
        'That Playwright argument is blocked by the local Filings launcher safety policy.',
      );
    }
  ...
  if (slotId === undefined) {
    throw new Error(
      'The local Filings launcher requires --slot=slot-NN (slot-00 through slot-09).',
    );
  }
...
export function parseLocalFilingsEnvFile(
  fileContents: string,
): Readonly<Record<string, string>> {
  if (typeof parseEnv !== 'function') {
    throw new Error(
      'The local Filings launcher requires Node.js 24 or newer.',
    );
  }
...
export function readLocalFilingsSlotPool(
  readLocalFile: LocalEnvFileReader = readFileSync as LocalEnvFileReader,
): Readonly<Record<string, string>> {
  let fileContents: string;
  try {
    fileContents = readLocalFile(LOCAL_FILINGS_ENV_FILE, 'utf8');
  } catch {
    throw new Error(
  ...
  return parseLocalFilingsEnvFile(fileContents);
}
...
function isSensitiveLauncherEnvironmentName(environmentName: string): boolean {
  ...
  return (
    ...
    normalizedName.startsWith('FILINGS_SLOT_') ||
    normalizedName.startsWith('BUSINESSES_LOCAL_') ||
    normalizedName.startsWith('FILINGS_LOCAL_') ||
    ...
    normalizedName === BUSINESSES_RESOURCE_SLOT_ENV ||
    normalizedName === BUSINESSES_EXPECTED_SLOT_ID_ENV ||
...
export function buildLocalPlaywrightArguments(
  ...
  if (
    ...
    (!path.isAbsolute(outputDirectory) ||
      !isStrictDescendant(LOCAL_FILINGS_RESULTS_ROOT, outputDirectory))
  ) {
    throw new Error(
      'The local Playwright output directory must be beneath filings/test-results/local/.',
    );
  }
  ...
  return [
    ...
    'test',
    '--config',
    path.join(filingsDirectory, 'playwright.config.ts'),
    '--workers=1',
...
export function runLocalFilingsTests(
  ...
  const localSlotPool = readLocalFilingsSlotPool();
...
if (require.main === module) {
  try {
    process.exitCode = runLocalFilingsTests();
  } catch (error) {
