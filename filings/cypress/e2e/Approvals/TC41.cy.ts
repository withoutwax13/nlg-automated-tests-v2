...
describe.skip("As AGS user, I want to be able to start approval workflow a specific item in Approvals", () => {
  it("Initiate test", () => {
    ...
    cy.get("@referenceId").then((referenceId) => {
      ...
      agsApprovalGrid.selectRowToReject("Reference ID", String(referenceId));
    });
  });
});
