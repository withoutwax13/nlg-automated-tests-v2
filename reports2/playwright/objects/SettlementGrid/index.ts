...
class SettlementGrid {
  ...
  private get defaultColumns() {
    return this.userType === "ags" ? AGS_SETTLEMENT_GRID_COLUMNS : MUNICIPAL_SETTLEMENT_GRID_COLUMNS;
  }
  ...
  async init() {
    await this.page.goto("/reports/settlementReport");
  ...
  async clickGenerateReportButton() {
    await this.getElement().generateReportButton().click();
    await waitForLoading(this.page, 10);
  }
