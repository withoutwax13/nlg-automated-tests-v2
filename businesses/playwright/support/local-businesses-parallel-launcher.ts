...
export function createLocalParallelRunPaths(
  ...
  if (Number.isNaN(now.getTime()) || !Number.isInteger(processId) || processId < 1) {
    throw new Error('Unable to create a valid local Businesses report run ID.');
  }
...
export function prepareParallelSlotEnvironment(
  ...
  childEnvironment.BUSINESSES_LOCAL_PARALLEL = 'true';
  childEnvironment.BUSINESSES_LOCAL_RESULT_FILE = resultFile;
...
function safeIpcMessage(
  value: unknown,
  slotId: string,
): LocalBusinessesResultIpcMessage | undefined {
  try {
    return parseLocalBusinessesResultIpcMessage(value, slotId);
  } catch {
    return undefined;
  }
}
...
function spawnSlot(
  ...
  process.stdout.write(`[${slotId}] Starting local Businesses tests.\n`);
  ...
  const resultFile = childEnvironment.BUSINESSES_LOCAL_RESULT_FILE as string;
...
export function formatParallelHeartbeat(
  ...
  const active = activeProgress.length
    ...
    : 'none';
  return (
    `Local Businesses progress: ${completedSlots}/${totalSlots} slots complete; ` +
...
export async function mergeLocalParallelReports(
  ...
  mergeEnvironment.PLAYWRIGHT_HTML_TITLE =
    `Businesses local parallel run ${runPaths.runId}`;
...
function printFinalSummary(result: ParallelRunResult): void {
  process.stdout.write('\nLocal Businesses parallel slot summary:\n');
...
async function runValidatedParallelBusinessesTests(
  options: ParallelLauncherArguments,
  runPaths: LocalParallelRunPaths,
): Promise<number> {
  ...
  process.stdout.write(
    `Starting ${options.slotIds.length} local Businesses slots with up to ${Math.min(
      options.maxParallel,
      options.slotIds.length,
    )} concurrent processes. Per-slot timeout: ${options.slotTimeoutMinutes} minutes.\n`,
  );
  process.stdout.write(`Local report run: ${path.relative(businessesDirectory, runPaths.rootDirectory)}\n`);
  ...
  try {
    ...
    if (interruptionExitCode !== undefined) {
      process.stderr.write('Local Businesses parallel run was interrupted.\n');
    ...
    if (reportResult.mergeExitCode === 0 && htmlReportExists) {
      process.stdout.write(
        `HTML report: ${path.relative(businessesDirectory, reportResult.htmlDirectory)}\n` +
    ...
    if (failed) {
      process.stderr.write(
        `Local Businesses parallel run failed${
          result.failedSlots.length > 0
    ...
    process.stdout.write('Local Businesses parallel run passed.\n');
    return 0;
  } finally {
    clearInterval(heartbeat);
...
export async function runParallelBusinessesTests(
  ...
  try {
    resetLocalBusinessesResults();
    return await runValidatedParallelBusinessesTests(
      options,
