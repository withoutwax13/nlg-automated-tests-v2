...
import {
  ...
  LOCAL_BUSINESSES_ENV_FILE,
  LOCAL_BUSINESSES_RESULTS_ROOT,
  parseLocalBusinessesEnvFile,
  parseLocalLauncherArguments,
  prepareLocalPlaywrightEnvironment,
  readLocalBusinessesSlotPool,
  resetLocalBusinessesResults,
  stripLocalSlotEnvironment,
} from './local-businesses-launcher';
import {
  EXPECTED_SLOT_ID_ENV,
  parseResourceSlot,
  RESOURCE_SLOT_ENV,
  type ResourceSlotPayload,
} from './resource-pool';

function createPayload(slotId = 'slot-03'): ResourceSlotPayload {
  return {
    VERSION: 1,
    SLOT_ID: slotId,
...
for (const managedArgument of [
  ...
  expectArgumentError(
    ...
    /blocked by the local Businesses launcher safety policy/,
  );
}
...
const parsedLocalSlotPool = parseLocalBusinessesEnvFile(localEnvFile);
...
const committedExamplePool = parseLocalBusinessesEnvFile(
...
const readLocalSlotPool = readLocalBusinessesSlotPool((filePath, encoding) => {
  requestedFilePath = filePath;
  requestedEncoding = encoding;
  return localEnvFile;
});
assert.equal(requestedFilePath, LOCAL_BUSINESSES_ENV_FILE);
...
const sourceEnvironment: NodeJS.ProcessEnv = {
  ...
  BUSINESSES_LOCAL_FAILURE_SCREENSHOTS: 'false',
  BUSINESSES_LOCAL_SINGLE: 'true',
  BUSINESSES_LOCAL_PARALLEL: 'true',
  BUSINESSES_LOCAL_RESULT_FILE: 'unsafe-result.json',
...
assert.equal(childEnvironment.BUSINESSES_LOCAL_FAILURE_SCREENSHOTS, 'true');
assert.equal(childEnvironment.BUSINESSES_LOCAL_SINGLE, undefined);
assert.equal(childEnvironment.BUSINESSES_LOCAL_PARALLEL, undefined);
assert.equal(childEnvironment.BUSINESSES_LOCAL_RESULT_FILE, undefined);
...
assert.equal(sourceEnvironment.BUSINESSES_LOCAL_FAILURE_SCREENSHOTS, undefined);
assert.equal(sourceEnvironment.BUSINESSES_LOCAL_SINGLE, undefined);
assert.equal(sourceEnvironment.BUSINESSES_LOCAL_PARALLEL, undefined);
assert.equal(sourceEnvironment.BUSINESSES_LOCAL_RESULT_FILE, undefined);
...
assert.match(playwrightArguments[3], /businesses[\\/]playwright\.config\.ts$/);
...
assert.throws(
  ...
  /must be beneath businesses\/test-results\/local/,
);

const temporaryRoot = mkdtempSync(
  path.join(tmpdir(), 'businesses-local-results-'),
);
try {
  const fakeBusinessesDirectory = path.join(temporaryRoot, 'businesses');
  ...
  assert.throws(
    ...
    /Another local Businesses run owns test-results/,
  );
  firstLease.release();
...
assert.equal(
  path.resolve(LOCAL_BUSINESSES_RESULTS_ROOT),
  path.resolve(__dirname, '..', '..', 'test-results', 'local'),
);

process.stdout.write('Businesses local launcher tests passed.\n');
